...
只有当用户输入明显与电商运营无关，并且无法合理推断为平台、商品、内容、用户、流量、趋势、召回相关任务时，才判断为 out_of_scope。




容错理解规则：
- 用户输入可能很短、口语化、只有关键词，甚至有错别字。
- 如果能合理推断它和电商运营、选品、平台运营、带货内容、热点趋势、用户召回有关，不要轻易判定为 out_of_scope。
- 如果用户只输入一个商品、类目、平台或运营关键词，可以先判为业务相关，并交给 requirement_agent 继续补全需求。
